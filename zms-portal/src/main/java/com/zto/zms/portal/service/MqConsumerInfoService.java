...
import com.zto.zms.service.repository.MqConsumerNumberRepository;
...
@Service
public class MqConsumerInfoService {

    @Autowired
    MqConsumerNumberRepository mqConsumerNumberRepository;

    public List<ConsumerInfoDTO> queryMaxConsumerInfo(ConsumerInfoQueryVO queryVo) {
        return this.mqConsumerNumberRepository.queryMaxConsumerInfo(queryVo, DateUtils.intervalTime(queryVo.getBeginTime(), queryVo.getEndTime())).stream()
                ...
                .map(item -> new ConsumerInfoDTO(item.getTime().getEpochSecond(), item.getClusterName(),
                        item.getConsumerGroup(), item.getName(), item.getValue()))
                .collect(Collectors.toList());
    }

    public List<ConsumerInfoDTO> queryLatestConsumerInfo(ConsumerInfoQueryVO queryVo) {
        return this.mqConsumerNumberRepository.queryLatestConsumerInfo((queryVo)).stream()
                .map(item -> new ConsumerInfoDTO(item.getTime().getEpochSecond(), item.getClusterName(),
                        item.getConsumerGroup(), item.getName(), item.getValue()))
                .collect(Collectors.toList());
    }
