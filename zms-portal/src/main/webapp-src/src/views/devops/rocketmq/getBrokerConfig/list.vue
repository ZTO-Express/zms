                ...
                <el-row :gutter="0">
                  ...
                  <el-col :span="5">
                    <el-form-item
                      label="节点"
                      prop="brokerAddr"
                      :rules="[{ required: true, message: '请输入节点', trigger: 'blur' }]"
                    >
                      <el-select
                        v-model="formData.brokerAddr"
                        v-bind="GLOBAL.select"
                        v-loading="SearchBrokerAddr.loading"
                        @focus="getBrokerAddr"
                      >
                        <el-option
                          v-for="item in SearchBrokerAddr.options"
                          :key="item.value"
                          :label="item.label"
                          :value="item.value"
                        ></el-option>
...
// 工具方法
import { getBrokerConfig } from '@/api/rocketmq-cmd'
// 搜索相关
...
export default {
  ...
  methods: {
    ...
    async loadTabledata(params = {}) {
      ...
      const { result } = await getBrokerConfig(params)
      const keys = Object.keys(result)
      const trans_result = []
      if (keys.length) {
        keys.map(key => {
          trans_result.push({ key, value: result[key] })
        })
      }
      this.maintable.data = trans_result
