                  ...
                  <el-col :span="5">
                    <el-form-item label="消费组" prop="consumerName">
                      <el-select
                        ...
                        v-bind="GLOBAL.select"
                        v-loading="SearchConsumerName.loading"
                        @focus="getConsumerOptions"
                      >
                        <el-option
                          v-for="item in SearchConsumerName.options"
                          :key="item.value"
                          :label="item.label"
                          :value="item.value"
                        ></el-option>
...
// 工具方法
import { statsAll } from '@/api/rocketmq-cmd'
// 搜索相关
...
export default {
  ...
  data() {
    return {
      maintable: {
        columns: [
          { type: 'index', width: 60 },
          { prop: 'topic', label: 'topic', width: 280 },
          { prop: 'consumerGroup', label: 'consumerGroup', width: 280 },
          { prop: 'accumulation', label: 'accumulation' },
          { prop: 'inTPS', label: 'inTPS', sortable: true },
          { prop: 'outTPS', label: 'outTPS', sortable: true },
  ...
  methods: {
    ...
    async loadTabledata(params = {}) {
      ...
      const res = await statsAll(params)
      if (res.result) {
