...
@Repository
public class KakfaConsumerNumberRepository {

    @Autowired
    private InfluxdbClient influxdbClient;

    public List<KafkaConsumerNumberInfo> queryMaxConsumerInfo(ConsumerInfoQueryVO queryVo, Long intervalTime) {
        String sql = "select max(value) as value,* from kafka_consumer_number_info where time >= {0} and time < {1}";
    ...
    public List<KafkaConsumerNumberInfo> queryLatestConsumerInfo(ConsumerInfoQueryVO queryVo) {
        String sql = "select * from kafka_consumer_number_info where time >= {0} and time < {1}";
        sql = StringFormatUtils.format(sql, queryVo.getBeginTime(), queryVo.getEndTime());
        StringBuilder sqlBuilder = new StringBuilder(sql);
        if (null != queryVo.getClusterName()) {
            sqlBuilder.append(" and clusterName='").append(queryVo.getClusterName()).append("'");
    ...
    private List<KafkaConsumerNumberInfo> getQueryResult(String sql) {
        QueryResult queryResult = influxdbClient.query(sql);
        InfluxDBResultMapper resultMapper = new InfluxDBResultMapper();
        return resultMapper.toPOJO(queryResult, KafkaConsumerNumberInfo.class);
    }
