...
@Repository
public class MqTopicInfoRepository {

    @Autowired
    private InfluxdbClient influxdbClient;

    public List<MqTopicInfo> queryMaxTopicInfo(MqTopicInfoQueryVO queryVo, Long intervalTime) {
        String sql = "select max(value) as value,* from mq_topic_info where time >= {0} and time < {1}";
    ...
    public List<MqTopicInfo> queryLatestTopicInfo(MqTopicInfoQueryVO queryVo) {
        String sql = "select * from mq_topic_info where time >= {0} and time < {1}";
        sql = StringFormatUtils.format(sql, queryVo.getBeginTime(), queryVo.getEndTime());
        StringBuilder sqlBuilder = new StringBuilder(sql);
        if (null != queryVo.getClusterName()) {
            sqlBuilder.append(" and clusterName='").append(queryVo.getClusterName()).append("'");
    ...
    public List<MqTopicInfo> getQueryResult(String sql) {
        QueryResult queryResult = influxdbClient.query(sql);
        InfluxDBResultMapper resultMapper = new InfluxDBResultMapper();
        return resultMapper.toPOJO(queryResult, MqTopicInfo.class);
    }
