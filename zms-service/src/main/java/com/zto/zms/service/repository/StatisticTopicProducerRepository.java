...
import com.zto.zms.service.influx.StatisticTopicProducerInfo;
...
@Repository
public class StatisticTopicProducerRepository {

    @Autowired
    private InfluxdbClient influxdbClient;

    public List<StatisticTopicProducerInfo> queryMaxStatisticTopicProducerInfo(StatisticTopicProducerQueryVO queryVo, Long intervalTime) {

        String sql = "select max(value) as value,* from statistic_topic_producer_info where time >= {0} and time < {1}";
    ...
    public List<StatisticTopicProducerInfo> queryLatestStatisticTopicProducerInfo(StatisticTopicProducerQueryVO queryVo) {

        String sql = "select * from statistic_topic_producer_info where time >= {0} and time < {1}";
        sql = StringFormatUtils.format(sql, queryVo.getBeginTime(), queryVo.getEndTime());
        StringBuilder sqlBuilder = new StringBuilder(sql);
        if (null != queryVo.getClientName()) {
            sqlBuilder.append(" and clientName='").append(queryVo.getClientName()).append("'");
    ...
    public List<StatisticTopicProducerInfo> getQueryResult(String sql) {
        QueryResult queryResult = influxdbClient.query(sql);
        InfluxDBResultMapper resultMapper = new InfluxDBResultMapper();
        return resultMapper.toPOJO(queryResult, StatisticTopicProducerInfo.class);
    }


    public List<InfluxTagInfo> queryTopicProducerTagInfo(String tagName, String clientName) {
        String sql = StringFormatUtils.format("show tag values from statistic_topic_producer_info with key={0} where clientName='{1}'",
                tagName, clientName);
